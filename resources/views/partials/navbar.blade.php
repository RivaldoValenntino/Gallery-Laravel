<nav x-data="{ scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 10 })" :class="{ 'border-b shadow-lg  dark:border-0': scrolled }"
    ...
    <div class="flex flex-wrap items-center justify-between max-w-full px-5 mx-auto">
                    ...
                    @auth
                        ...
                        <li>
                            <form action="{{ filament()->getLogoutUrl() }}" method="POST">
                                @csrf
                                <button
                                    class="block w-full px-4 py-2 text-sm text-left text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200">Sign
                                    Out</button>
                            </form>
                        </li>
                    @endauth
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-user">
            <ul
                ...
                <li>
                    <div class="px-4 py-2 {{ Request::is('photos*') ? ' bg-black rounded-full' : '' }}">
                        <a href="/photos"
                            class="block py-2 px-3 rounded hover:bg-gray-100 md:hover:bg-transparent {{ Request::is('photos*') ? 'text-white ' : 'text-gray-900' }} md:p-0">Home</a>
                    </div>
                </li>
                <li>
                    <div class="px-4 py-2 {{ Request::is('albums*') ? ' bg-black rounded-full' : '' }}">
                        <a href="/albums"
                            class="block py-2 px-3 rounded hover:bg-gray-100 md:hover:bg-transparent {{ Request::is('albums*') ? 'text-white' : 'text-gray-900' }} md:p-0">Albums</a>
                    </div>
                </li>
                ...
                <li>
                    <div class="px-4 py-2 {{ Request::is('categories*') ? ' bg-black rounded-full' : '' }}">
                        <a href="/categories"
                            class="block py-2 px-3 {{ Request::is('categories*') ? 'text-white' : 'text-gray-900' }} rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">Categories</a>
                    </div>
                </li>
                <li>
                    <div class="px-4 py-2">
                        <a href="/create"
                            class="block px-3 py-2 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">Create</a>
                    </div>
                </li>
            </ul>
        </div>
